...
export class NewsManagementComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if ((!this.selectedImage && !this.isEditMode) || (this.isEditMode && !this.imagePreview)) {
    ...
    request.subscribe({
      ...
      error: (error) => {
        ...
        if (error.status === 400) {
          ...
          this.errorMessage = 'Error inesperado al guardar la noticia, revisa el tamaño de la imagen o el formato';
        }
      },
      complete: () => {
  ...
  toggleNewsStatus(news: NoticeData): void {
    ...
    this.http.put(`http://localhost:8081/api/news/active/${news.id}`, updatedNews).subscribe({
      next: () => {
