...
import { Category, CountryUniversity, University } from 'src/app/domain/filter';
...
export class RankingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.rankingService.getYears().subscribe((data) => {
    ...
    this.categories = [
      ...
      {
        name: 'Communication & Textual Studies',
        subcategory: ['Languages & Linguistics'],
      },
      {
      ...
      {
        name: 'Enabling & Strategic Technologies',
        subcategory: [
          'Energy',
          'Materials',
          'Nanoscience & Nanotechnology',
      ...
      {
        name: 'Public Health & Health Services',
        subcategory: [
          'Nursing',
          'Public Health',
          'General & Internal Medicine',
    ...
    this.countryUniversity = [
      ...
      },
      {
        country: 'Libya',
        code: 'ly',
        universities: ['Omar Al-Mukhtar University'],
      },
      {
        country: 'Morocco',
        code: 'ma',
      ...
      },
      {
        country: 'Palestine',
        code: 'ps',
        universities: ['An-Najah National University', 'Birzeit University'],
      },
      ...
      },
      {
        country: 'Sudan',
        code: 'sd',
        universities: ['Alzaiem Alazhari University', 'Khartoum University'],
      },
  ...
  loadRanking(lastyear: number): void {
    this.rankingService.getTopRanking({ year: lastyear, size: 1000 }).subscribe(
      (data) => {
        ...
        this.test = this.processRankingData(data.content);
  ...
  loadFilters(year: number): void {
    this.rankingService.getFilters(year).subscribe((data) => {
  ...
  loadFilterCountries(year: number, country: string): void {
    this.rankingService
      .getFilterCountry({ year, country })
      .subscribe((data) => {
        this.test = data.content;
      });
  }

  loadFilterCountryUniversity(
    year: number,
    country: string,
    university: string,
  ): void {
    this.rankingService
      .getFilterCountryUniversity({ year, country, university })
      .subscribe((data) => {
        this.test = data.content;
      });
  }

  loadFilterCountryUniversityCategory(
    year: number,
    country: string,
    university: string,
    category: string,
    ...
    this.rankingService
      .getFilterCountryUniversityCategory({
        year,
        country,
        university,
        category,
  ...
  filterArabResearch() {
    if (this.isfilterArabResearch) {
      ...
      this.test = this.test
        .filter((item) => this.arabCountries.includes(item.working!))
        .map((item, index) => ({
          ...item,
          position: index + 1,
        }));
  ...
  // Combinar transformaciones en una sola operación
  private processRankingData(data: RankingTest[]): RankingTest[] {
    return data.map((item) => {
      const countryCode = this.codesCountries.find(
        (c) => c.code2 === item.codeCountry,
      );
      const workingCode = this.codesCountries.find(
        (c) => c.code2 === item.codeWorking,
      );
      const country = this.countries.find((c) => c.code === countryCode?.code);
      const working = this.countries.find((c) => c.code === workingCode?.code);

      return {
        ...item,
        codeCountry: countryCode?.code || item.codeCountry,
        codeWorking: workingCode?.code || item.codeWorking,
        country: country?.name || item.country,
        working: working?.name || item.working,
      };
    });
  }

  // Agregar funciones trackBy
  trackByYear(index: number, year: number): number {
    return year;
  }

  trackByRanking(index: number, item: RankingTest): number {
